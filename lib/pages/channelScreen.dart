...
class ChannelScreen extends StatefulWidget {
  ...
  const ChannelScreen({Key key, this.docId, this.channelDescription}) : super(key: key);
  @override
...
class _ChannelScreenState extends State<ChannelScreen> {
  ...
  _getPublicSnapshots() async {
    QuerySnapshot qn =
    await _firebaseFirestore.collection("public").where("channels",arrayContainsAny: [widget.docId]).get();
    return qn.docs;
  }
  @override
    ...
    return Scaffold(
      appBar: AppBar(
        title: Column(
          children: [
            Text(widget.docId),
            Text(widget.channelDescription,style: GoogleFonts.rajdhani(textStyle: TextStyle(fontSize: 15,fontWeight: FontWeight.w500, height: 0.8)),textAlign: TextAlign.justify,),
          ],
        ),
        centerTitle: true,
      ),
